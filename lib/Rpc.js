...
Rpc.getBlock = function(hash, cb) {
  ...
  bitcoreRpc.getBlock(hash, function(err,info) {
    ...
    if (info.result.height){
      ...
      var nSubsidyHalvingInterval = 1460;
      var XYC = 0.00000001;
      var r = 1.02338;
      var n = Math.floor(Math.min(info.result.height / nSubsidyHalvingInterval, 1080));
      info.result.reward = Math.floor(100 * Math.pow(r,n)) * XYC;
    }
